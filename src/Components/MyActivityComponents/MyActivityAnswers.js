		...
		<div className="comment rounded mb-1">
			<div className="d-flex flex-row p-2 px-3">
				<p className="text-secondary mb-0 mw-50">u/<span className="clickable-link">{props.username}</span> commented on
					{props.shortTitle == "" ? (
						<span className="text-dark clickable-link" style={{ cursor: "pointer" }}> {props.title}</span>
					) : (
						<span className="text-dark clickable-link" style={{ cursor: "pointer" }} data-toggle="tooltip" data-placement="top" title={props.title}> {props.shortTitle}</span>
					)}
				</p>
