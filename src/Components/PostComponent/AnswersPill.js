...
const answeredClasses = notAnsweredClassess + " bg-light-green";
	...
	return (
		<div className="d-flex align-items-center">
			<button className={isAnswered ? answeredClasses : answers > 0 ? hasAnswers : notAnsweredClassess} onClick={() => { console.log("action for answers click in progress"); }}>
				{isAnswered ? <span className="material-icons-outlined btn-outline-primary disable-pointer-events small-icon">mark_chat_read</span> : 
					answers > 0 ? <span className="material-icons btn-outline-success disable-pointer-events small-icon">chat_bubble_outline</span> : <span className="material-icons btn-outline-secondary disable-pointer-events small-icon">chat_bubble_outline</span>}
